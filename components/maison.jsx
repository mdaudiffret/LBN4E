...
const InfosSealed = ({ isAdmin, onAdminReveal, onReveal, revealCodes, user }) => {
  ...
  React.useEffect(() => {
    ...
    sb.from("user_codes").select("code_index").eq("user_id", user.id)
      .then(({ data: rows }) => {
        ...
        setFoundIdx(found);
        setInputs(prev => prev.map((v, i) => found.has(i) ? (codes[i] || v) : v));
  ...
  React.useEffect(() => {
    ...
    setInputs(prev => {
      const next = prev.map((v, i) => foundIdx.has(i) && codes[i] ? codes[i] : v);
      return next.every((v, i) => v === prev[i]) ? prev : next;
    });
  }, [codes, foundIdx]); // eslint-disable-line react-hooks/exhaustive-deps
